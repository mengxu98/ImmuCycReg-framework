...
ElVariableEnsembleSolve <- function(expressionMatrix,predictorIndices,targetIndices,rankThreshold=round((length(predictorIndices)/20)),
		ensembleSize=2000,minSampleSize=round((dim(expressionMatrix)[1])/5),maxSampleSize=4*round((dim(expressionMatrix)[1])/5),
		predictorSampleSizeMin=round(length(predictorIndices)/5),predictorSampleSizeMax=4*round(length(predictorIndices)/5),alpha=0.3,trace=TRUE,traceDetail=FALSE,...){
...
#									
#
elasticNetRankedSolve <- function(expressionMatrix,predictorIndices,targetIndices,alpha,rankThreshold,traceDetail,predictorSampleSizeMin,predictorSampleSizeMax,...){
	...
	for (targetIndex in targetIndices){
		...
		# Cross validate
		cross <-cv.glmnet(predictorsWithoutTarget,target,nfolds=nfolds,alpha=alpha,family="gaussian",standardize = FALSE,...)
		# Retrain optimal 
