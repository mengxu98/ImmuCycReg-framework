...
for (j in 1:length(target_genes_list)) {
  ...
  if (!dir.exists(paste0(pathSave, "corrgram/", target_gene))) {
    dir.create(paste0(pathSave, "corrgram/", target_gene), recursive = TRUE)
  }
  
  # select peaks according to distance and score correlation (population level)
  Y <- t(tcga_luad[target_gene, sample_name]) %>% as.data.frame()
  ...
  for (i in 1:length(peak_name)) {
    ...
    print(p)
    ggsave(paste(pathSave, "corrgram/", target_gene, "/", target_gene, "_", peak_name[i], "_corrgram.png", sep = ""),
           width = 4,
           height = 3
    )
  }
  ...
  write.csv(deg.data,
            file = paste(pathSave, "corrgram/", target_gene, "/", target_gene, "_peak_corr.csv", sep = ""),
            row.names = F
  )
  ...
  temp_res <- geneinfo_df[which(geneinfo_df[, "gene_name"] == target_gene), 
  ...
  if (dir.exists(paste0(pathSave, "volcano/")) == FALSE) {
    dir.create(paste0(pathSave, "volcano/"))
  }
  ...
  if (nrow(peak_seq_pos) > 0) {
    ...
    ggsave(paste0(pathSave, "volcano/", target_gene, "_volcano.pdf"),
           volcano_plot_pvalue,
           width = 6.5,
           height = 5
    )
    ...
    write.table(peak_seq_pos[, 1:3],
                file = paste0(pathSave, "BED/", target_gene, "_around_peak.bed"),
                sep = "\t",
                row.names = F,
                col.names = F,
                quote = F
...
write.table(peak_seq_pos_all[c(5:6, 1:4)], 
