...
# --------------------------------------------------
if(!file.exists(paste0(pathRead, "all_thresholded.by_genes_whitelisted.tsv"))){
  download.file("https://api.gdc.cancer.gov/data/7d64377f-2cea-4ee3-917f-8fcfbcd999e7", 
                paste0(pathRead, "all_thresholded.by_genes_whitelisted.tsv"))
}
tcga_cnv <- read.table(paste0(pathRead, "all_thresholded.by_genes_whitelisted.tsv"),
                       header = TRUE,
                       row.names = 1,
                       sep = "\t",
                       check.names = FALSE
...
# --------------------------------------------------
if(!file.exists(paste0(pathRead, "TCGA_ATAC_peak_Log2Counts_dedup_sample.gz"))){
  download.file("https://tcgaatacseq.s3.us-east-1.amazonaws.com/download/TCGA_ATAC_peak_Log2Counts_dedup_sample.gz", 
                paste0(pathRead, "TCGA_ATAC_peak_Log2Counts_dedup_sample.gz"))
}
...
# --------------------------------------------------
if(!file.exists(paste0(pathRead, "TCGA_ATAC_peak.all.probeMap"))){
  download.file("https://tcgaatacseq.s3.us-east-1.amazonaws.com/download/TCGA_ATAC_peak.all.probeMap", 
                paste0(pathRead, "TCGA_ATAC_peak.all.probeMap"))
}
genes_adj_peak <- read.table(paste0(pathRead, "TCGA_ATAC_peak.all.probeMap"),
                             header = TRUE,
                             row.names = 1,
                             check.names = FALSE,
                             sep = "\t"
...
# --------------------------------------------------
if(!file.exists(paste0(pathRead, "hg38.ncbiRefSeq.gtf.gz"))){
  download.file("https://hgdownload.soe.ucsc.edu/goldenPath/hg38/bigZips/genes/hg38.ncbiRefSeq.gtf.gz", 
                paste0(pathRead, "hg38.ncbiRefSeq.gtf.gz"))
}
gtf_df <- rtracklayer::import(paste0(pathRead, "hg38.ncbiRefSeq.gtf.gz")) %>% as.data.frame()
geneinfo_df <- dplyr::select(
  gtf_df,
  c(
    "seqnames",
    "start",
...
# Immune genes (accept genes list) need to be selected according to analysis!
