  ...
  if (target_gene %in% negative_genes) {
    ...
    if (length(high_exp_sample) > 0) {
      for (sample_i in seq_along(high_exp_sample)) {
        ...
        CNV_test <- F
        if (CNV_test) {
          up_reg_gene <- up_reg_gene[which(raw_tcga_cnv[up_reg_gene, target_sample] != 0)] # <U+73B0><U+5728>CNV<U+5927><U+4E8E>0<U+7684><U+8F6C><U+5F55><U+56E0><U+5B50><U+662F><U+5426><U+5408><U+7406><U+FF1F><U+9009><U+62E9><U+53EA><U+8981><U+662F><U+5BFB><U+627E>CNV<U+7684><U+5373><U+4F5C><U+4E3A><U+7A81><U+53D8>
