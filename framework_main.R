...
NIMEFI <- function(expressionMatrix,predictorIndices=NULL,targetIndices=NULL,GENIE=TRUE,SVM=TRUE,EL=TRUE,
		GENIETrace=TRUE,SVMTrace=TRUE,ELTrace=TRUE, # report output
		candidateSplitCount="ALL",treesInEnsembleCount=1000, # Genie parameters
		SVMPredSampleMin=20,SVMPredSampleMax=80,SVMExpSampleMin=20,SVMExpSampleMax=80,SVMRankThreshold=5,SVMEnsembleSize=600, # SVM parameters
		ELPredSampleMin=20,ELPredSampleMax=80,ELExpSampleMin=20,ELExpSampleMax=80,ELRankThreshold=5,ELEnsembleSize=600, # elastic net parameters
	...
	if(is.null(predictorIndices)){predictorIndices <- getIndicesOfGenesInMatrix(expressionMatrix)}
	# Set targetIndices to all if they are are not set
	if(is.null(targetIndices)){targetIndices <- getIndicesOfGenesInMatrix(expressionMatrix)}
	...
	if (SVM){
		...
		svmResult <- SVMVariableEnsembleSolve(expressionMatrix,predictorIndices,targetIndices,trace=SVMTrace,ensembleSize=SVMEnsembleSize, rankThreshold=SVMRankThreshold, 
	...
	if (EL){
		...
		elResult <- ElVariableEnsembleSolve(expressionMatrix,predictorIndices,targetIndices,trace=ELTrace,ensembleSize=ELEnsembleSize, rankThreshold=ELRankThreshold, 
