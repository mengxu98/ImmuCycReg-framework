...
library(survminer)
###
load('../data/2191/2191_492_TCGA.Rdata')
load("../data/LUAD_raw/luad-rsem-TPM-tcga-t_unnormlized.Rdata")
data_derived2 <- raw_tcga[rownames(data_derived), colnames(data_derived)]
###
    ...
    for (s in 1:length(seeds)) {
      ...
      for (p in 1:length(percent_genes)) {
        ...
        for (n in 1:length(nruns)) {
          ...
          cophenetic <- cbind.data.frame(percent_gene,seed, nrun, 
...
if (T) {
  ...
  dataset <- as.matrix(data_derived2)
  gene_no <- dim(dataset)[1]*0.25 
  gene_no <- 50 #50
  mads <- apply(dataset, 1, mad) # mad(x)  
  dataset <- dataset[rev(order(mads))[1:gene_no],]  
  dataset <- na.omit(dataset)
...
choose_columns=c('AJCC_METASTASIS_PATHOLOGIC_PM',
