...
#
#	Parameters	(required):
#		-- predictionTSV :  prediction file in tsv format (format: TF[TAB]TARGET_GENE[TAB]RANK -  with most confident gene at top)
#		-- goldTSV: gold standard in tsv format ((format: TF[TAB]TARGET_GENE[TAB]1/0] - 1 for true link, 0 for other links
#										
#	Parameters (optional):
# 
#		-- totalPredictionsAccepted [DEFAULT=100000]: maximum amount of predictions allowed
#
#	Returns:								
#		
#					an EvalClass object, which can be queried for AUROC and AUPR scores				
#
prepareEval <- function(predictionTSV, goldTSV,totalPredictionsAccepted=100000){
	...
	# Check arguments
	if ((!isWholeNumber(totalPredictionsAccepted)) || totalPredictionsAccepted < 1 ){stop("totalPredictionsAccepted should be an integer <0.")}
	
	# Next reduce the predictionTSV to the max allowed predictions if necessary
	if( length(pred[,1]) > totalPredictionsAccepted)  {pred <- pred[1:totalPredictionsAccepted,]}
	...
	if ( length(firstRow) < t){ odds <- (p-correct) / (t-length(firstRow))}
	else			{ odds <- 0}
...
#
#	Parameters	(required):
#		-- prediction: an evalClass object
#										
#	Parameters (optional):
# 
#
